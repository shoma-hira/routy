...
export function StoryPostViewer({ detail, isLoading, onClose, onSave, isSaved, isSaving }: { detail: StoryDetail; isLoading: boolean; onClose: () => void; onSave: () => void; isSaved: boolean; isSaving: boolean }) {
  ...
  if (isLoading) return <div className="flex min-h-[calc(100dvh-3.5rem)] items-center justify-center bg-zinc-950 text-sm text-white/70">コンテンツを読み込み中...</div>;

  return <section className="relative min-h-[calc(100dvh-3.5rem)] overflow-hidden bg-zinc-950 text-white" aria-label="投稿ストーリー">
    ...
    <div className="absolute right-4 top-7 z-30 flex gap-2">
...
function StoryContentSlide({ item, index, total, detail, failed, onImageError }: { item: StoryItem; index: number; total: number; detail: StoryDetail; failed: boolean; onImageError: () => void }) {
  ...
  return <article className={`relative flex min-h-[calc(100dvh-3.5rem)] items-end overflow-hidden ${image ? "bg-zinc-900" : "bg-gradient-to-br from-[#244636] via-[#52735d] to-[#1d2d29]"}`}>{image ? <Image src={text(item.image_url)} alt={`${spot(item)}の写真`} fill unoptimized sizes="(max-width: 640px) 100vw, 620px" className="object-cover" onError={onImageError} /> : null}<div className="absolute inset-0 bg-gradient-to-b from-black/30 via-transparent to-black/85" />{!image ? <span className="absolute left-6 top-24 rounded-full border border-white/30 bg-black/20 px-3 py-1 text-xs font-bold text-white/80">写真なし</span> : null}{content}</article>;
}
